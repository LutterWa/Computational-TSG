...
class OAgentSim:
    ...
    def modify(self, missile=None):  # 修改导弹初始状态
        if missile is None:
            missile = [0.,
                       ...
                       random.uniform(-20, 20) / RAD,
                       random.uniform(-30000., -10000),
                       random.uniform(-20000, 20000),
                       200]  # 0.时间,1.速度,2.弹道倾角,3.导弹x,4.导弹y,5.质量
            # missile = [0.,
            #            random.uniform(200, 300),
    ...
    def step(self, action=0., h=0.001):
        if self.Y[0] < 600:
            ...
            self.ab = action
            # self.ac = ac = 3 * v * qdot + math.cos(theta) * g + float(action)  # 制导指令
            tsg_n = 0.
            self.ac = ac = 2 * (tsg_n + 2) * v * qdot + math.cos(theta) * g + \
                           (tsg_n + 1) * (tsg_n + 2) * v * (q - action) / (R / v)  # 弹道成型
            self.alpha = alpha = (m * ac) / (Q * S * cl_alpha + self.P)  # 使用了sin(x)=x的近似，在10°以内满足这一关系
            if alpha > 15.0 / RAD:
                self.alpha = alpha = 15.0 / RAD
            elif alpha < -15.0 / RAD:
                self.alpha = alpha = -15.0 / RAD
...
if __name__ == '__main__':
    oAgent = OAgentSim()
    for i in range(1):
        oAgent.modify([0., 300., 0. / RAD, -20000., 20000, 200])
        done = False
        while done is False:
            done = oAgent.step(-179. / RAD, 0.01)  # 单步运行
        oAgent.plot_data()
